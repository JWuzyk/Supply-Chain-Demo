...
"""
Based on an airport, goal is to use data to generate a good simulation of the queue allowing us to determine expected wait times under different scenarios.
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                self.plot_container.altair_chart(
                    chart.properties(width=20 * data["time"].max()).encode(
                        y=alt.Y("queue_length", scale=alt.Scale(domain=(0, data["queue_length"].max())))
                    )
                )
...
class OnePlaneSimulation:
    ...
    def run(self):
        ...
        [
            Passenger(env, station, name, self.result_store, self.walk_dist, self.check_dist) for name in names
        ]  # init passengers
...
class Modeller:
    ...
    def model_arrival_time(self, arrival_times):
        ...
        with arrival_model:
            ...
            model_viz = pm.model_to_graphviz(arrival_model)

        return {
            "alpha": summary.loc["alpha", "mean"],
            "scale": summary.loc["scale", "mean"],
            "summary": summary,
            "plot": plot,
            "model_viz": model_viz,
        }
...
class DataDisplay:
    ...
    def plot_wait_times(self, df):
        chart = (
            ...
            .properties(title="Wait Times", width=200)
        )
        return chart
    ...
    def plot_arrival_times(self, df):
        chart = (
            ...
            .properties(title="Arrival Times", width=200)
        )
        return chart
    ...
    def plot_processing_time(self, df):
        chart = (
            ...
            .properties(title="Check Durations", width=200)
        )
        return chart
    ...
    def plot_queue_length(self, df):
        chart = (
            ...
            .properties(title="Queue Length", width=200)
        )
        return chart

    def combine_charts(self, charts, columns):
        return alt.vconcat(
            alt.hconcat(chart for chart in charts[i * columns : (i + 1) * columns])
            for i in range(len(charts) // columns)
        )
    ...
    def show(self):
        ...
        if fit:
            ...
            if self.arrival_results:
                st.markdown(
                    f"Arrivals estimates: shape: { self.arrival_results['alpha']}, scale: { self.arrival_results['scale']}"
                )
                [arrival_fig.add_subplot(p) for p in self.arrival_results["plot"].flatten()]
                st.graphviz_chart(self.arrival_results["model_viz"])
            st.pyplot(arrival_fig)

            processing_fig = plt.figure(1, 2)
            if self.processing_results:
                st.markdown(
                    f"Processing estimates: shape: {self.processing_results['alpha']}, scale: {self.processing_results['scale']}"
                )
                [processing_fig.add_subplot(p) for p in self.processing_results["plot"].flatten()]
                # st.graphviz_chart(self.processing_results["model_viz"])
...
class Settings:
    def show(self):
        ...
        with st.expander("Detailed Settings"):
            self.settings["walk_a"] = st.number_input(
                "Walk shape: Controls how spread out arrivals are", min_value=1, format="%i", value=5
            )
            self.settings["walk_scale"] = st.number_input(
                "Walk Scale: Controls how long it takes for passengers to arrive", min_value=0, format="%i", value=1
            )
            self.settings["check_a"] = st.number_input(
                "Check shape: Controls how spread out processing times are", min_value=1, format="%i", value=1
            )
            self.settings["check_scale"] = st.number_input(
                "Check Scale: Controls how long checks take.", min_value=0, format="%i", value=1
            )
